...
// This function resets or initializes the demotable.
async function resetEventtable() {
    const response = await fetch("/initiate-Eventtable", {
        method: 'POST'
    });
    const responseData = await response.json();

    if (responseData.success) {
        const messageElement = document.getElementById('EventresetResultMsg');
        messageElement.textContent = "Event initiated successfully!";
        fetchEventTableData();
    } else {
        alert("Error initiating Calendar!");
    }
